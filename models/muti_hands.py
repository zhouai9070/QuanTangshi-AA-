# -*- coding: utf-8 -*-
"""
Created on Sat Jul 13 16:25:06 2019
...
class multihead_attention(nn.Module):
    ...
    def __init__(self, num_units, num_heads=6, dropout_rate=0.25, causality=False):
    ...
    def forward(self, queries, keys, values):#输入的应该是个三维的向量
        # keys, values: same shape of [N, T_k, C_k]
        # queries: A 3d Variable with shape of [N, T_q, C_q]

        # Linear projections
        Q = self.Q_proj(queries)  # (N, T_q, C)(16*216*300)
        K = self.K_proj(keys)  # (N, T_q, C)(16*216*300)
        V = self.V_proj(values)  # (N, T_q, C)(16*216*300)
        ...
        outputs = torch.bmm(Q_, K_.permute(0, 2, 1))  # (h*N, T_q, T_k)  # scale dot product attention


        # Scale
        outputs = outputs / (K_.size()[-1] ** 0.5)  # scale dot product attention
        ...
        condition = key_masks.eq(0.).float()  # scale dot product attention
        outputs = padding * condition + outputs * (1. - condition) # scale dot product attention
        ...
        outputs = outputs * query_masks  # scale dot product attention

        # Dropouts
        outputs = self.output_dropout(outputs)  # (h*N, T_q, T_k)

        # Weighted sum
        outputs = torch.bmm(outputs, V_)  # (h*N, T_q, C/h) # scale dot product attention
        weight = outputs
...
class feedforward(nn.Module):
    def __init__(self, in_channels, num_units=[256, 300]):#输入的维度
...
"""if __name__ == '__main__':
    num_units = 512
    inputs = Variable(torch.randn((300, 512)))#输入二维向量
    #outputs = position_encoding(num_units)(inputs)#
    outputs = multihead_attention(num_units)(inputs, inputs, inputs)
