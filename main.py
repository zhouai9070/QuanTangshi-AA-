...
def main():
    ...
    """
    # save file
    config.mulu = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')  # 格式化时间
    config.save_dir = os.path.join(config.save_direction, config.mulu)   # 路径拼接
    if not os.path.isdir(config.save_dir):  # 判断对象是否为一个目录
